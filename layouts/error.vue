<template>
  <div class="h-screen bg-white flex items-center">
    <div
        ...
        <p class="text-2xl md:text-3xl font-light leading-normal">
          {{
            error.statusCode === 404
              ? $t('errorPage.sorryWeCouldNot')
              : $t('errorPage.anErrorOccurred')
          }}
        </p>
        <p class="mb-8">
          {{ $t('errorPage.butDontWorry') }}
        </p>

        <a
          class="bg-brand-500 hover:bg-brand-600 text-white py-3 px-6 rounded-full shadow-xl transform duration-300 hover:-translate-y-1"
          :href="localePath('/')"
        >
...
<style lang="postcss">
.nav.absolute {
  background: linear-gradient(270deg, #0acffe 0, #495aff);
  position: relative;
}
</style>
